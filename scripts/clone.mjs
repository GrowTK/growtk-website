...
if (!url) {
  console.error("Usage: node scripts/clone.mjs <url> [--slug name] [--name \"Business Name\"] [--pages 12] [--images 60] [--no-apply]");
  process.exit(1);
}
...
/*
 * A client does not always have their own website. A Yelp page, a Viator
 * listing, a Google Business profile: same crawl mechanics, but the domain is
 * a directory serving thousands of unrelated businesses, not the client's
 * site. Two things follow, both below: never treat the directory's own nav
 * as the client's site structure or wander into its other listings via
 * same-host links, and never take its og:site_name/logo/colors as the
 * client's brand identity.
 */
const DIRECTORY_HOSTS = [
  "yelp.com", "tripadvisor.com", "viator.com", "google.com", "facebook.com",
  "instagram.com", "opentable.com", "airbnb.com", "booking.com",
  "getyourguide.com", "expedia.com", "thumbtack.com", "angi.com", "houzz.com",
  "foursquare.com", "linkedin.com",
];
const isDirectory = DIRECTORY_HOSTS.some((d) => host === d || host.endsWith(`.${d}`));

if (isDirectory && !flag("slug", null)) {
  console.error(
    `${url} is a directory/profile page (${host}), not the client's own site. ` +
    `The slug can't be guessed from a platform hostname ("${host.split(".")[0]}" is not the business) ` +
    `-- pass --slug <the-actual-business-slug> explicitly.`,
  );
  process.exit(1);
}
...
while (frontier.length && results.size < MAX_PAGES) {
  ...
  for (const f of fetched) {
    ...
    // A directory's "same host" links are its own other listings and site nav,
    // not the client's pages -- one page is the whole crawl.
...
/** JSON-LD is written by the business's own listing, not the platform's chrome, so it survives on a directory page. */
const jsonLdName = (() => {
  let found = null;
  $('script[type="application/ld+json"]').each((_, el) => {
    ...
    try {
      ...
      for (const node of nodes) {
        if (typeof node?.name === "string" && node.name.trim()) { found = node.name.trim(); break; }
      }
...
/*
 * The h1 fallback below is a heuristic and directory pages break it in
 * practice: tested live against a real Yelp listing, the first h1 on the
 * page was a photo-gallery heading ("Business Photos"), not the business
 * name. --name exists so a human can just say what it is rather than the
 * tool guessing wrong with no way to correct it.
 */
const name = flag("name", null) || (isDirectory
  // og:site_name/application-name/title-host are the PLATFORM's identity on a
  // directory page (would resolve to "Yelp"), never the business's -- only
  // JSON-LD (written per-listing) or the visible h1 can be trusted here.
  ? jsonLdName || clean($("h1").first().text()) || null
  : jsonLdName || meta('meta[property="og:site_name"]') || meta('meta[name="application-name"]') ||
    clean($("title").text()).split(/[|\-·]/)[0].trim() || host);
const tagline = isDirectory ? clean($("h1").first().text()) || "" : meta('meta[property="og:title"]') || clean($("title").text()) || "";
...
// A directory page's colors/theme-color are the platform's own design system,
// not the business's -- skip the scan entirely rather than brand a site red
// because the source happened to be a Yelp listing.
if (!isDirectory) {
  $("[style]").each((_, el) => noteColor($(el).attr("style")));
  $("style").each((_, el) => noteColor($(el).html()));
}
const sheets = isDirectory ? [] : $('link[rel="stylesheet"]').map((_, el) => abs($(el).attr("href"))).get().filter(Boolean).slice(0, 5);
for (const css of await pool(sheets, 5, async (u) => { try { return await (await get(u, "text/css")).text(); } catch { return ""; } })) {
  noteColor(css);
}
const themeColor = isDirectory ? null : meta('meta[name="theme-color"]');
...
// A directory's own footer ("Follow Yelp on Instagram") looks identical to a
// business's real social links, so this can't be trusted on a directory page.
const socials = {};
...
// Both are the PLATFORM's on a directory page, so skip rather than brand the
// client's site with Yelp's red burst or Viator's mark.
const faviconUrl = isDirectory ? null :
  abs($('link[rel="apple-touch-icon"]').attr("href")) ||
  abs($('link[rel="icon"]').last().attr("href")) ||
  abs("/favicon.ico");
const logoUrl = isDirectory ? null : (() => {
  for (const sel of [
...
const inlineSvgLogo = !isDirectory && !logoUrl && $("header svg, nav svg").first().length > 0;
...
// A directory page's header/nav/footer is the PLATFORM's site structure
// (categories, login, app download), not the client's -- there is no real
// nav to harvest from a single listing page.
const navItems = isDirectory ? [] : linksIn("header, nav");
const footerItems = isDirectory ? [] : linksIn("footer");
...
await writeFile(join(outDir, "plan.md"),
  `# ${name || "(name not found, check the listing by hand)"} build plan\n\nSource: ${url}\n` +
  (isDirectory
    ? `Source type: directory/profile listing (${host}), not an owned site. No real nav, brand color, fonts, or logo exist to inherit -- design the site from scratch using only what's below plus your own judgement.\n`
    : "") +
