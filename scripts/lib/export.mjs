...
export const STRIP = [
  // .claude/ and CLAUDE.md are kept: they let whoever picks up the exported
  // repo, us or the client, keep building it with Claude Code.
  ".scrape", ".factory", ".security", "supabase",
  "AGENTS.md", "scripts", "ideas", ".env.example", ".npmrc",
  ...
  // The whole-site template registry. A chosen one is already copied into
  // content/ and app/(site)/ by the provisioning pipeline's apply_template
  // step before this runs locally; the registry itself never ships.
  "templates",
  // NOT content/demo.ts: the section library and page templates import its
  // types and default values at compile time (139 blocks + 6 templates deep,
  // since "Grow the section library" 7f1e6a1). Stripping it breaks every
  // client build with TS2307 on every file that never got its demo import
  // swapped for real content. No secrets in it, just verified stock photo
  // URLs and placeholder copy, so it ships harmlessly instead.
];
...
export function buildExport(root) {
  ...
  try {
    ...
    for (const file of walkFiles(work)) {
      ...
      if (/@\/app\/\(factory\)|components\/sections\/(?:sidebar|catalog\.tsx|theme|font-select)|sections-showcase|showcase-fonts/.test(src)) {
