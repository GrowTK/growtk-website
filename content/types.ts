...
/**
 * Anything the booking widget can be opened against: a tour, a class, a
 * table, an appointment slot. `Tour` (content/demo.ts) and `BookableTour`
 * (components/sections/contact/contact-03.tsx) already satisfy this
 * structurally, so real per-site content needs no rewrite to use it.
 */
export type Activity = {
  title: string;
  /** Falls back to a slugified title when absent (see components/booking). */
  id?: string;
  duration?: string;
  price?: string;
  capacity?: number;
