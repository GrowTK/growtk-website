...
Before picking any block: read `components/sections/catalog.tsx`. One file, every
block's `label` (layout description plus a "Best for..." fit clause), `file`,
`component`, `props`. Decide what to use from that alone, then open only the
specific files you picked to confirm the exact prop shape. Do NOT glob or read
through `components/sections/**` to browse what's available, that is the slow
path this file exists to replace.
