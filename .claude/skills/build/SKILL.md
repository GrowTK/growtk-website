...
**Pick blocks from `components/sections/catalog.tsx`, not by reading the
component directory.** One file, every block's `label` (layout description
plus a "Best for..." fit clause), `file`, `component` and `props`. Read that
once, decide what to use, then open only the specific files you picked to
confirm the exact prop shape. This is the whole difference between a fast
build and one that reads through 139 component files to browse them.
