...
export function Nav09({
  ...
  return (
    <header
      className={cn(
        "sticky top-0 z-50 border-b border-border/70 bg-background/85 shadow-[0_1px_0_0_rgba(0,0,0,0.02)] backdrop-blur-md transition-transform duration-300 ease-out motion-reduce:transition-none",
    ...
    >
      <nav aria-label="Primary" className="mx-auto flex h-16 max-w-7xl items-center gap-8 px-6">
        ...
        >
          {logoSrc ? (
            <img src={logoSrc} alt={logoAlt ?? brandName} loading="lazy" decoding="async" className="h-8 w-auto" />
          ) : null}
